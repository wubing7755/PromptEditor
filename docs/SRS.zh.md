...
PromptLib 是一个纯 C11 命令行应用程序，用于在本地文件支持的库中保存、组织、检索、浏览和优化
prompt 模板。本文档定义了 PromptLib 的功能与非功能需求。

### 1.2 范围

- 一个 CLI 可执行文件 (`pp`)，包含 18 个子命令。
- 一个可通过 CMake `find_package` 安装使用的可复用 C 库 (`promptlib_core`)。
...
PromptLib 是独立的 CLI 工具，无网络依赖。完全在本地文件上运行。同时作为可安装的 C 库发布。
...
| **Downstream Developer** | 链接 `PromptLib::promptlib_core` |
...
```
Title  可安装 CMake 包
Desc   cmake --install → 下游可 find_package(PromptLib CONFIG REQUIRED)。
```
...
| NFR-008 | 🔴 | 依赖 | `promptlib_core` 零外部链接时依赖 |
...
- `src/cli.c` — 已实现的行为
- `include/promptlib/example.h` — 公共 API
