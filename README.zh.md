...
- **标签管理** 使用 `pp tag add/remove/list` 增量管理标签
- **归档管理** 使用 `pp archive` 管理已删除的 prompt（恢复 / 永久清除）
- **重建索引** 使用 `pp reindex` 从源元数据文件重建索引
- **统计信息** 使用 `pp stats` 查看库的完整统计
- **导出 / 导入 / 备份** 完整库操作
...
| `tag` | 增量标签管理（`add`、`remove`、`list`） |

### 归档

| 命令 | 说明 |
|------|------|
| `archive` | 管理已归档 prompt（`list`、`restore`、`purge`） |
...
| 命令 | 说明 |
|------|------|
| `reindex` |  从各 prompt 的 `metadata.tsv` 重建 `index.tsv`（支持 `--dry-run` 预览） |
| `stats` | 显示库统计信息 — prompt 数、文件夹数、标签数、磁盘使用（支持 `--json`） |
